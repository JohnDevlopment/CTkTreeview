...
class CTkListbox(customtkinter.CTkScrollableFrame):
    def __init__(
        ...
        **kwargs,
        ...
        # The foreground color for buttons when the associated item is
        # selected
        self.select_color = (
        ...
        # The color for buttons when the mouse hovers over them. This
        # is used to set "hover_color" for buttons
        self.hover_color = (
        ...
        if not font:
            ...
            if isinstance(font, customtkinter.CTkFont):
                ...
                self.font = customtkinter.CTkFont(font) # pyright: ignore

        # The foreground color for buttons. If "default" or omitted,
    ...
    def select(self, index):
        """select the option"""
        # Every button has a foreground color, which is the default
        # color for it. However, when the item is selected, its
        # associated button is changed to its "select color", denoted
        # by "hover_color" in the constructor.
    ...
    def bind(self, key, func, add="+"):
        super().bind(key, lambda e: func(e), add=add) # pyright: ignore
        self._parent_frame.bind(key, lambda e: func(e), add=add) # pyright: ignore
        self._parent_canvas.bind(key, lambda e: func(e), add=add) # pyright: ignore
